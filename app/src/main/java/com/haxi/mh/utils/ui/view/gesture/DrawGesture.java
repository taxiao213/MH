 ...
 */
public class DrawGesture extends View {
    ...
    private List<PointGesture> list;// 装有各个view坐标的集合
    private List<Pair<PointGesture, PointGesture>> lineList;// 记录画过的线

    /**
     * 手指当前在哪个Point内
     */
    private PointGesture currentPoint;
    /**
     * 用户绘图的回调
     */
    ...
    public DrawGesture(Context context, List<PointGesture> list, GestureCallBack callBack) {
        super(context);
        paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
        //        bitmap = Bitmap.createBitmap(480, 854, Bitmap.Config.ARGB_8888); // 设置位图的宽高
        bitmap = Bitmap.createBitmap(UIUtil.getScreenDispaly(context)[0], UIUtil.getScreenDispaly(context)[1], Bitmap.Config.ARGB_8888);
        canvas = new Canvas();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // 得到当前移动位置是处于哪个点内
                PointGesture pointAt = getPointAt((int) event.getX(), (int) event.getY());
                //代表当前用户手指处于点与点之前
                ...
                if (pointAt == null || currentPoint.equals(pointAt)
                    ...
                    Pair<PointGesture, PointGesture> pair = new Pair<>(currentPoint, pointAt);
                    lineList.add(pair);
     ...
     */
    private PointGesture getPointAt(int x, int y) {

        for (PointGesture point : list) {
            // 先判断x
    ...
    private void clearScreenAndDrawList() {
        ...
        for (Pair<PointGesture, PointGesture> pair : lineList) {
