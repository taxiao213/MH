...
import com.haxi.mh.model.db.DBTableColumn;
...
public class DBMigrationHelper {
    ...
    private static DBMigrationHelper migrationHelper;
    ...
    public static DBMigrationHelper getInstance() {
        if (migrationHelper == null) {
            synchronized (DBMigrationHelper.class) {
                if (migrationHelper == null) {
                    migrationHelper = new DBMigrationHelper();
                }
            }
        }
    ...
    private void restoreData(Database db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        ...
        for (int i = 0; i < daoClasses.length; i++) {
            ...
            ArrayList<DBTableColumn> list = new ArrayList<>();//表中多出的参数
            for (int j = 0; j < daoConfig.properties.length; j++) {
                ...
                if (getColumns(db, tempTableName).contains(columnName)) {
                    ...
                    try {
                        ...
                        list.add(new DBTableColumn(columnName, type));
            ...
            try {
                ...
                if (list != null && list.size() > 0) {
                    ...
                    for (int j = 0; j < list.size(); j++) {
                        DBTableColumn tableColumn = list.get(j);
                    ...
                    if (arrayList != null && arrayList.size() > 0) {
                        ...
                        LogUtils.e("DBOpenHelper  ---", "更新语句 stringBuilder ===" + stringBuilder.toString());
                        if (arrayList.contains("IS_READ_MESSAGE")) {
                            ...
                            LogUtils.e("DBOpenHelper  ---", "更新语句 st ===" + st.toString());
                        }
                    }
                    LogUtils.e("DBOpenHelper  ---", "添加的参数 list.size =" + list.toString());
                }
            } catch (Exception e) {
                LogUtils.e("DBOpenHelper  ---", "错误" + e.getMessage() + "  添加的参数 list.size =" + list.toString());
            }
        }
        LogUtils.e("DBOpenHelper----", "restoreData 恢复数据");
    }
        ...
        if (TextUtils.equals(buildType, "debug")) {
            for (Class<? extends AbstractDao<?, ?>> daoClass : daoClasses) {
                try {
                    ...
                    LogUtils.e("DBOpenHelper---- createTables debug e == " + e.getMessage());
                    e.printStackTrace();
                }
            }
        } else {
            for (int i = 0; i < daoClasses.length; i++) {
                ...
                try {
                    ...
                    LogUtils.e("DBOpenHelper  ---createTables release sql== " + createTableStringBuilder.toString());
                } catch (Exception e) {
                    LogUtils.e("DBOpenHelper  ---createTables release e == " + e.getMessage());
                }
            }
        }
    }
        ...
        if (TextUtils.equals(buildType, "debug")) {
            for (Class<? extends AbstractDao<?, ?>> daoClass : daoClasses) {
                try {
                    ...
                    LogUtils.e("DBOpenHelper---- debug deleteTables e == " + e.getMessage());
                    e.printStackTrace();
                }
            }
        } else {
            for (int i = 0; i < daoClasses.length; i++) {
                try {
                    ...
                    db.execSQL(dropTableStringBuilder.toString());
                    LogUtils.e("DBOpenHelper---- release deleteTables sql== " + dropTableStringBuilder.toString());
                } catch (Exception e) {
                    LogUtils.e("DBOpenHelper---- release deleteTables e == " + e.getMessage());
                }
            }
        }
    }
    ...
    private void generateTempTables(Database db, Class<? extends AbstractDao<?, ?>>... daoClasses) {
        ...
        for (int i = 0; i < daoClasses.length; i++) {
            ...
            try {
                db.execSQL(createTableStringBuilder.toString());
                LogUtils.e("DBOpenHelper  ---", "generateTempTables create sql== " + createTableStringBuilder.toString());
            } catch (Exception e) {
                LogUtils.e("DBOpenHelper  ---", "generateTempTables create e== " + e.getMessage());
            }
            if (properties != null && properties.size() > 0) {
                Iterator<String> iterator = properties.iterator();
                while (iterator.hasNext()) {
            ...
            try {
                db.execSQL(insertTableStringBuilder.toString());
                LogUtils.e("DBOpenHelper  ---", "generateTempTables insert sql== " + insertTableStringBuilder.toString());
            } catch (Exception e) {
                LogUtils.e("DBOpenHelper  ---", "generateTempTables insert e== " + e.getMessage());
            }
        }
    }
    ...
    private static List<String> getColumns(Database db, String tableName) {
        ...
        try {
            ...
            LogUtils.e("DBOpenHelper  ---getColumns", e.getMessage());
            e.printStackTrace();
        } finally {
            if (cursor != null)
                cursor.close();
